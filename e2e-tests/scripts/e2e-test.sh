...
wait_for_service() {
    ...
    echo "INFO - Waiting cluster startup on port $port"
...
switch_wal() {
    local port=$1
    exec_sql ${port} "SELECT pg_switch_xlog();"
    exec_sql ${port} "SELECT pg_switch_xlog() from gp_dist_random('gp_id');"
}

restore_backup() {
    local port=$1
    echo "INFO - Getting latest backup name..."
    BACKUP_NAME=$(exec_docker ${PRIMARY_MASTER_CONTAINER} "wal-g backup-list --config ${WALG_CONFIG}" | grep backup_20 | head -1 | awk '{print $1}')
    ...
    exec_docker ${RESTORE_MASTER_CONTAINER} "gpstop -a"  
    ...
    exec_docker ${RESTORE_MASTER_CONTAINER} "sed -i \"s/^archive_command='wal-g seg wal-push %p --content-id=%c --config \/tmp\/wal-g.yaml'/#&/\" /data/master/gpseg-1/postgresql.conf"
    exec_docker ${RESTORE_MASTER_CONTAINER} "gpssh -h segment1 -h segment2 'find /data -name postgresql.conf -exec sed -i \"s|^archive_command=.*wal-g.*|#&|\" {} \;'"
    ...
    exec_docker ${RESTORE_MASTER_CONTAINER} "gpstart -a"
...
echo "INFO - Checking data on port ${GP_MASTER_PORT}"
primary_data=$(get_table_data ${GP_MASTER_PORT})

echo "INFO - Creating backup full backup on primary cluster"
...
echo "INFO - Check standby Greenplum cluster"
wait_for_service ${GP_RESTORE_MASTER_PORT}

echo "INFO - Restore full backup on standby cluster"
restore_backup ${GP_RESTORE_MASTER_PORT}

echo "INFO - Checking data on port ${GP_RESTORE_MASTER_PORT}"
sleep 60
standby_data=$(get_table_data ${GP_RESTORE_MASTER_PORT})

echo "INFO - Comparing data between primary and standby clusters"
echo "INFO - Primary data:"
echo "$primary_data"
echo "INFO - Standby data:"
echo "$standby_data"
if [ "$primary_data" = "$standby_data" ]; then
    echo "INFO - Test completed successfully"
    exit 0
else
    echo "ERROR - Test failed during data comparison"
    exit 1
fi
